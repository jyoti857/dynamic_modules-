...
import {shareTypeObj} from './constants';
...
export const editConexionMapper = data => {
  if (!Lo.isEmpty(data)) {
    const payload = {
      ...
      ind_shared_users: [],
      ind_linked_in: data.LinkedIn,
      ind_contact_preference: data.ContactPreference,
      ind_status: data.Status,
      //-----------
    ...
    if (data.Users.length > 0) {
      data.Users.forEach(user => {
        if (!payload.ind_shared_users.includes(user.UserId)) {
          payload.ind_shared_users.includes(user.UserId);
        }
      });
    }
    return payload;
...
export const individualConexionPayloadMapper = form => {
  ...
  if (form.ind_first_name) {
    payload.Name = form.ind_first_name;
  }
  if (form.ind_middle_name) {
    payload.MiddleName = form.ind_middle_name;
  }
  if (form.ind_last_name) {
    payload.LastName = form.ind_last_name;
  }
  if (form.ind_title) {
    payload.Title = form.ind_title;
  }
  if (form.ind_suffix) {
    payload.Suffix = form.ind_suffix;
  }
  if (form.ind_select_oraganisation) {
    payload.ParentConexionId = form.ind_select_oraganisation;
  }
  if (form.ind_job_title) {
    payload.JobTitle = form.ind_job_title;
  }
  ...
  if (form.ind_business_fax) {
    payload.BusinessFaxNumber = form.ind_business_fax;
  }
  ...
  if (form.ind_home_phone) {
    payload.HomeTelephoneNumber = form.ind_home_phone;
  }
  if (form.ind_home_phone_2) {
    payload.Home2TelephoneNumber = form.ind_home_phone_2;
  }
  if (form.ind_hom_fax) {
    payload.HomeFaxNumber = form.ind_hom_fax;
  }
  ...
  if (form.ind_linked_in) {
    payload.LinkedIn = form.ind_linked_in;
  }
  if (form.ind_contact_preference) {
    payload.ContactPreference = form.ind_contact_preference;
  }
  if (form.ind_status) {
    payload.Status = form.ind_status;
  }
